...
- **Groupes d’agents** : flotte `entreprise` (système, défaut métier) + équipes projet optionnelles ; délégation restreinte aux membres du groupe. Ops : **Équipes projet** (`/gestion/equipes`) pour ouvrir un contexte (chat, missions, brief). Admin : **Équipes** (`/administration/equipes`) avec onglets Identité / Composition / Politique / Mémoire. Si `memory_scope=group` (défaut des équipes projet), le prompt **n’injecte pas** la science d’entreprise (mémoire globale, faits, historique de toutes les missions) : uniquement notes d’équipe, hors-périmètre, et la consigne. `inherit_shared` peut réinjecter un extrait du global. `memory_scope=enterprise` (flotte métier) conserve la mémoire workspace. Mémoire partagée : Moteur IA → Mémoire partagée.
