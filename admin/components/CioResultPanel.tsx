...
export default function CioResultPanel({
  ...
  const inAppDeliverables = useMemo(() => deliverablesForMissionPanel(String(result || "")), [result]);
  ...
  return (
    <section className={shellClass} aria-label="Réponse finale du CIO">
      ...
      {inAppDeliverables.length > 0 ? (
        <div className="border-b border-emerald-100 bg-gradient-to-b from-emerald-50/50 to-white px-4 py-4 sm:px-5">
          <p className="text-xs font-bold uppercase tracking-wide text-emerald-900">Livrables opérationnels</p>
          <p className="mt-1 text-[11px] leading-relaxed text-slate-600">
            {inAppDeliverables.length} pièce{inAppDeliverables.length > 1 ? "s" : ""} lisible
            {inAppDeliverables.length > 1 ? "s" : ""} directement dans Korymb — tableaux, courriers et scripts prêts à l&apos;emploi.
          </p>
          {jobId ? (
            <DeliverableAccessHub
              jobId={jobId}
              deliverablesMarkdown={String(result || "")}
            ...
            {inAppDeliverables.map((item) => (
              ...
              >
                <div className="flex flex-wrap items-start justify-between gap-2">
                  ...
                  <button
                    type="button"
                    onClick={() => setPreview({ title: item.title, body: item.body })}
                    className="rounded-lg border border-violet-200 bg-violet-50 px-2.5 py-1 text-[11px] font-bold text-violet-900 hover:bg-violet-100"
                  >
                    Afficher
                  </button>
                </div>
