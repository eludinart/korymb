...
export default function GlobalStatusBar() {
  ...
  const hasAlert = backendDown || dbDown || metricsDegraded || isBudgetExceeded || isAlert;
  const hasRunningJobs = running > 0;

  if (!hasAlert && !hasRunningJobs) {
    return null;
  }

  if (collapsed && !hasAlert) {
    return (
      <div className="flex flex-wrap items-center justify-between gap-2 rounded-xl border border-violet-200 bg-violet-50/80 px-3 py-2">
        <p className="text-xs font-semibold text-violet-950">
          {running} mission{running > 1 ? "s" : ""} en cours
          {inflight > 0 ? ` · ${fmt(inflight)} tokens en vol` : ""}
        </p>
        <Link href="/briefing" className="text-xs font-bold text-violet-800 hover:underline">
          Voir le briefing →
        </Link>
      </div>
    );
  ...
  if (collapsed && hasAlert) {
    return (
      <div className="space-y-2">
        ...
        {metricsDegraded ? (
          <p className="rounded-xl border border-amber-200 bg-amber-50 px-3 py-2 text-xs text-amber-900">
            Métriques en retard — rechargez ou consultez le{" "}
            <Link href="/briefing" className="font-bold underline">
              briefing
            </Link>
            .
          </p>
        ) : null}
        {isBudgetExceeded || isAlert ? (
          <p className="rounded-xl border border-red-200 bg-red-50 px-3 py-2 text-xs font-extrabold text-red-950">
            {isBudgetExceeded ? "Budget IA dépassé" : "Seuil d'alerte tokens atteint"} —{" "}
